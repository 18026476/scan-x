...
Write-Host "=== PATCH: Force UTF-8 decoding for Process.run/start output ==="
...
foreach ($f in $files) {
  ...
  # Patch Process.run(...) where stdoutEncoding/stderrEncoding is missing
  $src = [regex]::Replace(
    $src,
    "Process\.run\(\s*([^,]+)\s*,\s*([^\)]+?)\)",
    {
  ...
  # Patch Process.start(...) where stdoutEncoding/stderrEncoding is missing
  $src = [regex]::Replace(
    $src,
    "Process\.start\(\s*([^,]+)\s*,\s*([^\)]+?)\)",
    {
