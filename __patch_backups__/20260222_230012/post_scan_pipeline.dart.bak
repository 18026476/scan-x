...
class PostScanPipeline {
  static Future<void> handleScanComplete(
    ...
    try {
      ...
      // 1) Bool keys (REAL scanx_* + legacy for comparison)
      final boolKeys = <String>[
        'scanx_notifyNewDevice','scanx_notifyUnknownDevice','scanx_notifyRouterVuln','scanx_notifyIotWarning',
        'scanx_notifyHighRisk','scanx_notifyScanCompleted','scanx_notifyAutoScanResults',
        'scanx_alertSoundEnabled','scanx_alertVibrationEnabled','scanx_alertSilentMode',
      ...
      // 2) Int keys (avoid the "int is not bool?" crash)
      final sens1 = p.getInt('scanx_alertSensitivity');
      final sens2 = p.getInt('alertSensitivity');
      parts.add('scanx_alertSensitivity=' + (sens1 == null ? 'null' : sens1.toString()));
      parts.add('alertSensitivity=' + (sens2 == null ? 'null' : sens2.toString()));
