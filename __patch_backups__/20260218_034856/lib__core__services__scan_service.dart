...
class ScanService {
  ...
  String _sanitizeText(String s) {
    ...
    const junk = <String>[
      ...
      'ÃƒÆ’Ã†€™Ãƒ€šÃ‚¢ÃƒÆ’Ã‚¢Ãƒ¢Ã¢€š¬Ã…¡Ãƒ€šÃ‚¬',
      'ÃƒÆ’Ã†€™Ãƒ€šÃ‚¢ÃƒÆ’Ã‚¢Ãƒ¢Ã¢€š¬Ã…¡',
      'ÃƒÆ’Ã†€™Ãƒ€šÃ‚¢',
    ];
    for (final j in junk) {
      t = t.replaceAll(j, '');
  ...
  List<DetectedHost> _parseNmapOutput(String stdoutStr) {
    ...
    for (final raw in lines) {
      ...
      if (portMatch != null && currentAddress != null) {
        ...
        final rawService = portMatch.group(3) ?? '?';
      final service = TextSanitizer.normalizeServiceGuess(TextSanitizer.normalizeUi(rawService));
